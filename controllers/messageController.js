...
exports.deleteFavoriteMessage = async (req, res) => {
  ...
  try {
    ...
    user.favorites = user.favorites.filter(favId => favId.toString() !== messageId);
    await user.save();

    res.json({ success: true, message: "Message removed from favorites" });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
};
